...
impl Api {
    pub async fn new(username: &str, password: &str, address: &str) -> Result<Self, error::Error> {
        ...
        headers.insert("Origin", address.parse()?);

        let addr = push_own! { address, "/api/v2/auth/login" };
        let request = client.post(&addr).form(&[("username", username), ("password", password)]).headers(headers);
    ...
    pub async fn get_torrent_properties(&self, hash: &Hash) -> Result<Torrent, error::Error> {
        let addr = push_own! {self.address, "/api/v2/torrents/properties?hash=", &hash};
        ...
        let torrent: Torrent = serde_json::from_slice(&res)?;
        Ok(torrent)
    }
